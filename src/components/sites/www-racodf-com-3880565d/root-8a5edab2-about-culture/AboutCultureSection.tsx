...
export function AboutCultureSection() {
  return (<div id="about-culture" className="about-part scroll-mt-20">
    ...
    <section className="fade-in-up" aria-labelledby="culture-service"><div className="mb-6 flex items-center gap-3"><span className="h-[3px] w-8 bg-primary" aria-hidden="true" /><h4 id="culture-service" className="text-xl font-semibold text-gray-900 md:text-2xl">服务理念与商业模式</h4></div><div className="grid gap-4 md:grid-cols-3">{['四海合作', '天下共赢', '供应链管理'].map((item) => (<article key={item} className="rounded-xl border border-gray-200 bg-gray-50 p-6 text-center text-lg font-semibold text-gray-900 transition-all duration-200 hover:-translate-y-1 hover:border-primary-mid hover:bg-white hover:shadow-card-hover lg:p-8">{item}</article>))}</div></section>
  </div>);
}
